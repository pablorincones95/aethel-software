...
export async function submitContactForm(
  ...
  // If Supabase is not configured, log for local dev and return success
  if (!supabaseUrl || !supabaseKey) {
    console.log("[Contact Form] Supabase not configured — lead data received:", {
      name,
      email,
      organization,
      service,
  ...
  try {
    ...
    // 1. Save lead into dedicated contact_leads table
    ...
    if (dbError) {
      console.error("[Contact Form] DB Insert error:", dbError)
      // Fallback: don't fail user experience if table doesn't exist yet
    }
    ...
    return { success: false, error: "Error procesando solicitud. Intenta nuevamente." }
  }
}
